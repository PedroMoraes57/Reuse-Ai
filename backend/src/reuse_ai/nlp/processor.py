...
_NORMALIZED_TOKEN_REPLACEMENTS = {
...
def _expand_normalized_text(text: str) -> str:
    if not text:
        return text
    expanded_tokens: list[str] = []
    for token in text.split():
        replacement = _NORMALIZED_TOKEN_REPLACEMENTS.get(token, token)
...
class PortugueseTextPreprocessor:
    ...
    def process(self, text: str) -> ProcessedText:
        ...
        normalized = _expand_normalized_text(normalized)
